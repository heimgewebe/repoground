# RepoGround Systemkarte

## 1. Systemachsen

Diese Karte ordnet die Kernmodule von RepoGround den architektonischen Systemachsen zu, basierend auf dem aktuellen Implementierungsstand im Repository (`merger/repoground/`).

### Achse 1: Build & Ingestion (Scan → Extract → Chunk → Bundle)
- **`merger.repoground.core.merge`**
...
- **`merger.repoground.core.chunker`**
...
- **`merger.repoground.core.extractor`**
...
- **`merger.repoground.core.pr_schau_bundle`**
...
- **`merger.repoground.retrieval.query_core`**
  ...
  - **Runtime-Nutzer:** API-Service (`merger.repoground.service.app`), CLI (`cmd_query.py`), Evaluierung.

- **`merger.repoground.retrieval.eval_core`**
...
- **`merger.repoground.retrieval.index_db`**
...
- **`merger.repoground.core.range_resolver`**
...
- **`merger.repoground.architecture.graph_index`**
...
- **`merger.repoground.service.app`**
  - **Zweck:** FastAPI Backend für die RepoGround-Dienste, Agenten-Integration und UI-Bereitstellung.
  - **Inputs:** HTTP Requests, persistierte RepoGround Bundles / Atlas Metarepos.
...
Die Systemkommunikation läuft über JSON Schemas in `merger/repoground/contracts/`.
...
- **Cross-Repo / Federation (Roadmap Phase 4):** Föderations-Module sind
  inzwischen vorhanden — `merger/repoground/core/federation.py`,
  `merger/repoground/retrieval/federation_query.py`,
  `merger/repoground/cli/cmd_federation.py` sowie die Contracts
  ...
  `docs/roadmap/repoground-master-roadmap.md` (Phase 4).
