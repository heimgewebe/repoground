...
| Fleet-Metadaten beschrieben zunächst ein fremdes Repo: `.ai-context.yml` (primary_language `bash`, Entrypoints `scripts/tools/*-pin.sh`, Runbook `docs/runbook.md` – alles inexistent) und `.wgx/profile.yml` (`repo: heimgewebe/tools`, `class: bash-tooling`, `lang: shell`) waren Kopien aus `heimgewebe/tools`; eine Zwischenkorrektur führte noch die aktive Lenskit-/RepoLens-Identität | `.ai-context.yml`, `.wgx/profile.yml`, `AGENTS.md` vs. realer Repo-Inhalt | **behoben** | T015 bindet die aktiven Flächen an RepoGround: `profile: repoground`, `class: knowledge-compiler`, RepoGround-Tags und -Taskausgaben; der auf Commit `a7466bf2c3cb7fa3b261bfb2d008612abc0f85b1` gepinnte externe `heimgewebe/wgx`-Guard sowie der lokale Metadaten-Regressionstest sind grün. Historische Lenskit-/RepoLens-Belege werden nicht umgeschrieben. |
