...
| `lenskit verify` war ein Platzhalter-No-op ("Verify command placeholder. Use pr-schau-verify for now.") | `cli/main.py:219-221` (alt) | **behoben** | an den vorhandenen contract-gestützten Verifier (`pr-schau.v1`) verdrahtet: neuer Library-Entry `pr_schau_verify.run_verify(bundle, level)` (Exit-Code statt `sys.exit`), Subparser mit `bundle`/`--level`; Standalone-`main()` + bestehende Verifier-Tests unverändert grün; E2E-Smoke geprüft |
...
| Atlas-CLI-Definitionen dupliziert (manuelle Sync-Pflicht, Drift-Risiko) | `cli/main.py:131-132` (NOTE) und `cli/rlens.py` | **offen** | getrackt: Atlas-Subparser über einen gemeinsamen Registrar zentralisieren (Muster `register_*_commands`), um Drift strukturell auszuschließen |
