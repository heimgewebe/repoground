...
| `<stem>.citation_map.jsonl` | `citation_map_jsonl` | `navigation_index` | `derived` | `core.citation_map` (`produce_citation_map`) | _(geplant: Query/Context/Agent Evidence Pack)_ | `citation-map.v1.schema.json` | Ja (wenn canonical_md/chunk_index kohärent) | Belegadressierung |
| `<stem>.agent_reading_pack.md` | `agent_reading_pack` | `navigation_index` | `derived` | `core.agent_reading_pack` (`produce_agent_reading_pack`) | LLM-Agents (Einstiegsdokument), Mensch | - (role_only; Markdown) | Ja | Kompakter Navigations-/Lese-Einstieg für Agents |
...
| `<stem>.python_call_graph.json` | `python_call_graph_json` | `navigation_index` | `derived` | `architecture.call_graph` über `core.merge` | MCP `find_references`, `get_callers`, `get_callees`, Agent Reading Pack | `python-call-graph.v1.schema.json` | Ja, Single-Repo-Bundle; aus `public-share` ausgeschlossen | Statische S0/S1-Aufrufnavigation; S1 nur bei genau einem lokal aufgelösten Ziel, keine Runtime- oder Vollständigkeitsbehauptung |
| `<name>.observed_call_overlay.json` | _(kein Manifest-Rollenname; eigenständiges Operator-Artefakt)_ | `runtime_observation` | `observation` | `architecture.observed_call_overlay` über CLI `repoground observed-calls produce` (führt als einziger Einstiegspunkt Zielcode aus, **nie** aus `core.merge`) | CLI `repoground observed-calls callers|callees` (`core.observed_call_navigation`) | `python-observed-call-overlay.v1.schema.json` | Nein (kein Bundle-Artefakt) | Laufgebundene S2-Beobachtung genau eines Kommandos in genau einer Umgebung an genau einer Quellrevision; keine Vollständigkeits-, Erreichbarkeits-, Dead-Code- oder Coverage-Aussage und keine Anhebung statischer S0/S1-Evidenz |
...
6. **Authority/Canonicality-Felder (Phase 1 + 3.5):**
   ...
   **Im Schema als Zukunftsform per-role-constrained, aber nicht vom `bundle-manifest.v1`-Producer emittiert:**
   ...
   - `delta_json` — lebt weiterhin im separaten PR-Schau-Bundle (`core.pr_schau_bundle`). Zusätzlich emittiert `write_reports_v2` im `bundle-manifest.v1`-Pfad optional `<stem>.delta.json`, wenn ein validiertes `pr-schau-delta.v1`-Mapping (`delta_meta`) vorliegt. Es bleibt `diagnostic_signal` / `diagnostic` und darf nicht als kanonischer Inhalt interpretiert werden.
