...
| **rLens CLI Client Blueprint** | keine Tests; Blueprint-only | Dokumentiert geplanten read-only HTTP-Client, Host-Modell Heim-PC/Heimserver, Base-URL-/Token-Modell und Sicherheitsgrenzen | **Blueprint dokumentiert; Tests geplant.** | Implementierung und Tests folgen in separatem PR |
