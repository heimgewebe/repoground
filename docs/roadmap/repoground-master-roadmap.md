...
5. Atlas bleibt Paralleltrack und blockiert RepoGround Evidence Addressing nicht.
...
Aktueller PR:
- [x] `docs/roadmap/repoground-master-roadmap.md`
...
- [x] Citation-Map-CLI-Producer plus Real-Dump-Proof
  - **Producer implementiert und gehärtet (2026-05-14):**
    - `merger/repoground/core/citation_map.py` (pure Funktionen + IO-Adapter).
    - CLI: `repoground citation produce <bundle_manifest>` mit `--json`- und `--output`-Option.
...
- [x] Citation-Readiness-Validator plus Real-Dump-Proof
  - `merger/repoground/core/citation_validate.py` implementiert (Konsument/Readiness-Gate, kein Producer).
  - CLI: `repoground citation validate <bundle_manifest>` mit `--json`-Option.
...
- Optionale Rust-/Bash-Strukturevidenz ist als commit-/manifestgebundene
  Navigation umgesetzt (`language_structure_json`); Default/Breitaktivierung
  bleibt bis zu einem revisionsgebundenen Benefit-vs-Fallback-Gate aus
...
Scope: prüft die C1-Kompatibilität bestehender Contracts unter `merger/repoground/contracts/`
...
Scope: ausschließlich `merger/repoground/contracts/bundle-manifest.v1.schema.json`:
...
Scope: `ARTIFACT_AUTHORITY_REGISTRY` in `merger/repoground/core/merge.py` emittiert
...
- Geänderte/ergänzte Dateien:
  - `merger/repoground/core/anti_hallucination_lint.py` (reine Funktionen + IO-Adapter)
  - `merger/repoground/cli/cmd_governance.py` (`repoground governance lint`, Dispatch in `cli/main.py`)
  - `merger/repoground/tests/test_anti_hallucination_lint.py` (25 Tests)
...
- Geänderte/ergänzte Dateien:
  - `merger/repoground/core/agent_export_gate.py` (additives Lesen + Gate-Bedingung)
  - `merger/repoground/tests/test_agent_export_gate.py` (C2.5-Tests)
...
- Geänderte/ergänzte Dateien:
  ...
  - `merger/repoground/retrieval/eval_diagnostics.py` (`DOES_NOT_PROVE`-Konstante +
    Emission in `generate_report`)
  - `merger/repoground/core/anti_hallucination_lint.py` (`DEFERRED_BOUNDARY_CONTRACTS`
    jetzt leer; Mechanismus erhalten)
  - `merger/repoground/tests/test_anti_hallucination_lint.py`,
...
- Geänderte/ergänzte Dateien:
  - `merger/repoground/core/anti_hallucination_ast_lint.py` (neue Lint-Engine, reine Funktionen)
  - `merger/repoground/cli/cmd_governance.py` (`repoground governance ast-lint`, experimentell)
  - `merger/repoground/cli/main.py` (Dispatch für das neue Subkommando)
  - `merger/repoground/tests/test_anti_hallucination_ast_lint.py` (25 Tests)
...
- Geänderte/ergänzte Dateien:
  - `merger/repoground/core/authority_upgrade_registry.py` (neu — Registry + Validierung + Matching)
  - `merger/repoground/core/anti_hallucination_ast_lint.py` (Finding trägt `authority`/`sink`;
    Report trägt `declared_upgrades`/`declared_upgrade_count` + volle Registry-Sicht; stage `C2.9`)
  - `merger/repoground/cli/cmd_governance.py` (Human-Report zeigt declared upgrades)
  - `merger/repoground/tests/test_anti_hallucination_ast_lint.py` (25 → 39 Tests)
...
- **Registry-Eintrag (v0, genau einer):** `AuthorityUpgrade(rule=L4,
  ...
  sink=resolve_canonical_md, file_suffix=merger/repoground/core/merge.py, reason=…)`. File- und Sink-scoped (kein `symbol`): nur in
...
`merger/repoground/core/doc_freshness.py` + CLI `repoground doc-freshness
inspect|update` klassifizieren Drift und regenerieren die Ansicht
`docs/_generated/doc-freshness.md`. Pilot: der `merger/repoground/repoground-build-spec.md`
...
- RepoGround = Knowledge Compiler / Evidence Runtime
...
- [x] `docs/proofs/citation-map-artifact-fit.md`
PR 2:
- [x] `citation-map.v1.schema.json` (`merger/repoground/contracts/citation-map.v1.schema.json`)
- [x] minimale Beispiele (`merger/repoground/contracts/examples/citation_map_minimal.jsonl`)
- [x] Schema-Test (`merger/repoground/tests/test_citation_map_schema.py`)
...
- [x] Citation-Readiness-Validator (`merger/repoground/core/citation_validate.py`, CLI `repoground citation validate`, Testabdeckung in `merger/repoground/tests/test_citation_validate.py` und `merger/repoground/tests/test_cli_citation.py`; Real-Dump-Proof erbracht mit aktuellem Dump, 594 Chunks, Status `ok`)
PR 4 (erledigt):

- [x] `merger/repoground/core/parity_gates.py` — Produktionsmodul mit `evaluate_parity_gates` und `ParityGateResult`. Gate-Semantik ist jetzt kanonisch und wiederverwendbar (nicht mehr nur Test-Helper).
- [x] PR 4b Basis: Real-Dump-Parser + CLI-Compare-Pfad
  - `merger/repoground/core/parity_state.py` erzeugt ein kanonisches State-Dict aus zwei realen Bundle-Manifests fuer `evaluate_parity_gates`.
  - CLI: `repoground parity compare LEFT_MANIFEST RIGHT_MANIFEST --json` mit standardisierten Exit-Codes (0/1/2).
...
- [x] repolens diagnostic parity hardening
  ...
  - E2E-Beleg gegen echte Bundle-Manifests via `build_parity_state`+`evaluate_parity_gates`: `merger/repoground/tests/test_parity.py::test_e2e_repolens_rlens_reach_diagnostic_parity` (alle 15 State-Flags True, 10 Artefakte verglichen, kein left/right-only). Beleg: `docs/proofs/repolens-rlens-diagnostic-parity-proof.md`.
...
- [x] CLI-Erzwingung und parity-relevantes CI-Gate
  - CLI-Erzwingung: `repoground parity enforce LEFT RIGHT --require {content,diagnostic}` (Default `diagnostic`) in `merger/repoground/cli/cmd_parity.py`, Exit-Codes 0/1/2, `--json`/`--include-state`. Policy ist profilabhaengig (content fuer degradierte Profile, diagnostic fuer vollwertige Hosts).
  ...
  - Tests: `merger/repoground/tests/test_cli_parity_compare.py` (enforce Policy-/Exit-Code-Faelle), `test_parity.py::test_e2e_parity_enforce_cli_on_real_bundles`.
...
PR 5 (docs-only): RepoGround service CLI Client Blueprint und Umsetzungspfad

- [x] Blueprint anlegen: `docs/blueprints/rlens-cli-client-blueprint.md`
- [x] Read-only RepoGround service CLI Client Basis implementiert (PR B)
...
- [x] Read-only Jobs/Logs (PR C)
  ...
  - Tests: `merger/repoground/tests/test_cli_rlens_client.py` deckt POST, Payload, URL-Encoding, Token-Header, Text-/JSON-Ausgabe und HTTP-Fehler ab.
- [x] Host-Profile (PR D)
  ...
  - [x] `repoground rlens-client profiles [--json]` listet Profile (redigiert; nur `base_url` und `token_env`-Name) und validiert Config strikt
...
- `merger/repoground/cli/serve.py` bleibt Service-Launcher und wird nicht umgedeutet.


PR 6 (Artifact Output Control Plane, docs-first):
- [x] `docs/blueprints/repoground-artifact-output-control-plane.md`
...
- Epistemische Korrektur: keine Auto-Claim-Bewertung (`supported/unsupported`); RepoGround adressiert Belege, bewertet sie nicht (`docs/blueprints/lenskit-output-optimierung-v1.md` AP F angepasst).
...
- Geänderte/ergänzte Dateien:
  ...
  - `merger/repoground/contracts/context-quality.v1.schema.json`
  - `merger/repoground/cli/cmd_context_quality.py` (Dispatch in `merger/repoground/cli/main.py`)
  - `merger/repoground/tests/test_context_quality.py`
  - `merger/repoground/tests/test_cli_context_quality.py`
...
- CLI: `repoground context-quality inspect <manifest> [--json] [--emit-artifact] [--output PATH]`.
...
- Validierung:
  ...
  - `python3.11 -m pytest merger/repoground/tests/test_context_quality.py merger/repoground/tests/test_cli_context_quality.py` (24 passed)
  - `python3.11 -m pytest merger/repoground/tests/test_output_health.py merger/repoground/tests/test_post_emit_health.py merger/repoground/tests/test_cli_bundle_health.py merger/repoground/tests/test_bundle_manifest_integration.py` (93 passed, keine Regression)
...
- Geänderte/ergänzte Dateien:
  - `merger/repoground/retrieval/eval_core.py` erweitert: `classify_miss()`, `build_miss_taxonomy()`
  - `merger/repoground/contracts/retrieval-eval.v1.schema.json` erweitert um `miss_taxonomy` (optional, backward-compatible)
  - `merger/repoground/tests/test_retrieval_eval.py` erweitert: B2-Tests inkl. `query_execution_error` und Contract-Shape-Guards
...
- Validierung:
  - `ruff check --select=F401,F811 --exclude='**/fixtures/**' .` — PASSED
  - `python3 -m pytest merger/repoground/tests/test_retrieval_eval.py -v` — 35 PASSED
  - `python3 -m pytest merger/repoground/tests/test_retrieval_eval.py -k "miss_taxonomy or classify_miss" -v` — selected tests PASSED
...
- Behoben (verifiziert: Python-/Repo-Testlauf 1456 passed / 1 skipped; zusätzlich wurden
  ...
  - `repoground verify` von Platzhalter-No-op auf echten contract-Verifier verdrahtet
...
- Offen (getrackt, bewusst nicht blind geändert):
  - ~~Atlas-CLI-Subparser-Duplikation `cli/main.py` ↔ `cli/rlens.py` zentralisieren.~~ **UMGESETZT** (2026-05-27): `register_atlas_commands` + `handle_atlas_command` in `merger/repoground/cli/cmd_atlas.py` als Single Source of Truth; beide Entry-Points konsumieren den Registrar. Beleg: `inconsistencies.md` §7 (Status "behoben").
