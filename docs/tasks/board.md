...
| TASK-SERVICE-002 | rLens Pre-Pull vor Merge (bounded repo-sync mutation) | done | `merger/lenskit/service/repo_sync.py`, `merger/lenskit/service/models.py`, `merger/lenskit/service/runner.py`, `merger/lenskit/cli/cmd_rlens_client.py`, `merger/lenskit/frontends/webui/{index.html,app.js}`, `merger/lenskit/tests/test_repo_sync.py`, `merger/lenskit/tests/test_service_runner_pre_pull.py`, `merger/lenskit/frontends/webui/tests/test_pre_pull_payload.js`, `docs/service-api.md` | Fast-forward-only, abschaltbar (`pre_pull`, Default an). Clone fehlender Repos und Auto-Restart des Self-Repos bleiben bewusst Nicht-Ziele. |
