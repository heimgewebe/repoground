...
| TASK-NOISE-002 | Real-Dump Noise Hygiene Runtime/Surface Proof | partial | `scripts/rlens-post-merge-surface-smoke.sh`, `merger/lenskit/tests/test_rlens_post_merge_surface_smoke.py`, `docs/proofs/output-noise-hygiene-real-dump-proof.md` | Container hat keinen systemd-User-Bus und keine reale merges-Directory; realer operator-host Dump nach `systemctl --user restart rlens` muss den erweiterten Smoke noch belegen. |
