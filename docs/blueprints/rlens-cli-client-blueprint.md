...
- `--pre-pull` (Default, aktiv) / `--no-pre-pull` steuern den Fast-Forward-only-Pre-Pull vor dem Scan (siehe `docs/service-api.md`, `pre_pull` / `bounded repo-sync mutation`). Beide Flags sind mutually exclusive; `pre_pull` wird immer explizit im Payload gesendet.
