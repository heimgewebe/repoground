...
- rLens Source Acquisition v1 (TASK-SERVICE-003): `--source-mode {local-current,local-ff,remote-snapshot}` ist die führende Quellensemantik; optional `--remote-ref` und `--remote-ref-policy {upstream,same-branch,default-branch}` für `remote-snapshot`. Bei gesetztem `--source-mode` leitet der Modus `pre_pull` ab (local-ff ⇒ true, sonst false) und der Payload trägt `repo_source_mode` (+ `remote_ref_policy`/`remote_ref` für remote-snapshot). Widersprüche werden über die zentrale Validierung (`validate_source_mode_request`, dieselben Regeln wie `/api/jobs`) **vor jedem Netzwerkzugriff** als `config_error` (Exit-Code 2) abgelehnt: `local-current --pre-pull`, `local-ff --no-pre-pull`, `remote-snapshot --pre-pull`, `local-ff --plan-only` (local-ff würde mutieren, plan-only verbietet Mutation), `--remote-ref` ohne `--source-mode remote-snapshot`, sowie eine explizite nicht-Default-`--remote-ref-policy` ohne `remote-snapshot`. `--remote-ref-policy` hat jetzt Default `None`; nur bei `remote-snapshot` wird sonst `upstream` in den Payload gesetzt, sodass lokale Modi kein inertes Feld senden (kein Job-Hash-Drift). `remote-snapshot --plan-only` ist erlaubt (Dry-Plan). Details: `docs/blueprints/rlens-source-acquisition-blueprint.md`, `docs/service-api.md` (*Source Acquisition*).
