...
from merger.repoground.adapters.atlas import AtlasScanner, render_atlas_md
...
def run_atlas_index(args: argparse.Namespace) -> int:
    ...
    from merger.repoground.atlas.registry import AtlasRegistry
    from merger.repoground.atlas.paths import resolve_atlas_base_dir, resolve_index_db_path
    ...
    try:
        ...
        def _do_scan():
            ...
            if not getattr(args, "no_index", False):
                try:
                    from merger.repoground.atlas.index import AtlasFTSIndex
                    from merger.repoground.atlas.paths import resolve_index_db_path
...
def _run_analyze_backup_gap(source_snapshot_id: str, backup_snapshot_id: str) -> int:
    from merger.repoground.atlas.diff import compute_snapshot_comparison
...
def _run_analyze_growth(source_snapshot_id: str, target_snapshot_id: str) -> int:
    from merger.repoground.atlas.diff import _load_inventory_index
    from merger.repoground.atlas.registry import AtlasRegistry
    from merger.repoground.atlas.paths import resolve_atlas_base_dir, resolve_artifact_ref
    import datetime
...
def register_atlas_commands(subparsers) -> None:
    ...
    (RepoGround entry point) and `cli/serve.py` (RepoGround service launcher) consume this
