...
from merger.repoground.core.context_quality import (
...
def test_non_bundle_manifest_blocks(tmp_path):
    ...
    assert report["projection_status"] == "blocked"
    assert any("not a supported RepoGround or legacy bundle manifest" in e for e in report["errors"])
...
def test_docs_do_not_claim_b2_is_implemented():
    ...
    roadmap = (_REPO_ROOT / "docs" / "roadmap" / "repoground-master-roadmap.md").read_text(encoding="utf-8")
