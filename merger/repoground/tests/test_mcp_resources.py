...
from merger.repoground.core.mcp_resources import (
    RepoGroundMcpResourceError,
    list_mcp_resources,
    read_mcp_resource,
    resource_templates,
)
...
def test_mcp_resource_list_exposes_concrete_snapshot_resources(tmp_path):
    ...
    assert "repoground://snapshot/demo/manifest" in uris
    assert "repoground://snapshot/demo/canonical" in uris
    assert "repoground://snapshot/demo/reading-pack" in uris
    assert "repoground://snapshot/demo/health" in uris
...
def test_mcp_read_manifest_resource_carries_context_and_content(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/manifest",
...
def test_mcp_read_bundle_manifest_artifact_role_is_available(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/bundle_manifest",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_file_bundle_root_rejects_invalid_json_manifest(tmp_path):
    ...
    result = read_mcp_resource("repoground://snapshot/bad/manifest", bundle_root=bad)
...
def test_mcp_artifact_resource_blocks_paths_outside_bundle_root(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/escape_attempt",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_relative_escape_paths(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/relative_escape",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_symlink_escape_paths(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/symlink_escape",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_integrity_mismatch(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/mutable",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_missing_integrity_metadata(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/no_integrity",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_invalid_sha_metadata(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/bad_sha",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_accepts_uppercase_sha_metadata(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/upper_sha",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_existing_minimal_artifact_without_integrity_is_blocked(tmp_path):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/citation_map_jsonl",
        bundle_root=bundle["manifest"].parent,
    )
...
def test_mcp_artifact_resource_blocks_oversized_content(tmp_path, monkeypatch):
    ...
    result = read_mcp_resource(
        "repoground://snapshot/demo/artifact/oversized",
        bundle_root=bundle["manifest"].parent,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "file:///tmp/demo",
        "repoground://snapshot/demo/unknown",
        "repoground://snapshot/demo/artifact",
        "repoground://snapshot/../manifest",
        "repoground://snapshot/demo/artifact/../secret",
    ],
)
