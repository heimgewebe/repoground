...
def test_profile_output_mode_plan_is_machine_readable():
    ...
    assert public_default["excluded_roles"] == [
        "sqlite_index",
...
def test_placeholder_for_availability_model_import():
    from merger.repoground.core.repobrief_availability import AVAILABILITY_VALUES
...
def test_availability_model_required_and_freshness(tmp_path):
    import datetime
    import json
    from merger.repoground.core.repobrief_availability import snapshot_availability_model
    data = {
...
def test_graph_availability_reports_validation_unavailable_as_degraded(tmp_path, monkeypatch):
    import json
    from merger.repoground.core import repobrief_availability as availability
