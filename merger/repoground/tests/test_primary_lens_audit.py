...
from merger.repoground.core.lens_audit import (
    DOES_NOT_ESTABLISH,
    KIND,
    VERSION,
...
def test_primary_lens_audit_rejects_absolute_paths():
    with pytest.raises(ValueError, match="repo-relative"):
        audit_primary_lenses(["/tmp/lenskit/merger/repoground/core/lenses.py"])


def test_primary_lens_audit_rejects_parent_traversal_paths():
    with pytest.raises(ValueError, match="parent traversal"):
        audit_primary_lenses(["../merger/repoground/core/lenses.py"])
    with pytest.raises(ValueError, match="parent traversal"):
...
@pytest.mark.parametrize(
    "bad_path",
    [
        "/tmp/lenskit/merger/repoground/core/lenses.py",
        ".",
        "   ",
        "../merger/repoground/core/lenses.py",
