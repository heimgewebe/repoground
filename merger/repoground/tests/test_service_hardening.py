...
from merger.repoground.service.models import JobRequest, Artifact
...
def test_get_server_version_logs_debug_on_git_failure(monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="merger.repoground.service.app"):
...
def test_app_module_import_survives_git_failure_during_server_version_init():
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import merger.repoground.service.app as a; print(a.SERVER_VERSION)"],
        capture_output=True,
        text=True,
        env=env,
        cwd=str(repo_root),
