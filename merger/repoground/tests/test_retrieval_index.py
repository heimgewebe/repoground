...
def test_index_ingest_diagnostics(tmp_path, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="merger.repoground.retrieval.index_db"):
        index_db.build_index(dump_path, chunk_path, db_path)
...
def test_build_index_closes_connection_on_error(tmp_path):
    ...
    with patch("merger.repoground.retrieval.index_db.sqlite3") as mock_sqlite3:
