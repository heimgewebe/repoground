...
def _write_manifest(
    tmp_path: Path, *, git_commit: str = "head", canonical: bool = False
) -> Path:
    artifacts = []
        ...
        json.dumps(
            {
                "kind": (
                    ...
                    if canonical
                    else "repolens.bundle.manifest"
                ),
                "version": "2.0" if canonical else "1.0",
                "run_id": "fixture",
                "generator": {
                    "runtime": {
...
def test_manifest_bundle_accepts_canonical_and_legacy_identity(tmp_path):
    legacy_dir = tmp_path / "legacy"
    canonical_dir = tmp_path / "canonical"
    legacy_dir.mkdir()
    canonical_dir.mkdir()
    legacy = _write_manifest(legacy_dir)
    canonical = _write_manifest(canonical_dir, canonical=True)

    _, legacy_roles, _ = audit._load_manifest_bundle(legacy)
    _, canonical_roles, _ = audit._load_manifest_bundle(canonical)
    assert set(legacy_roles) == set(audit.REQUIRED_BUNDLE_ROLES)
    assert set(canonical_roles) == set(audit.REQUIRED_BUNDLE_ROLES)
