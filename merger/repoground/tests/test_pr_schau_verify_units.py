...
from merger.repoground.cli.pr_schau_verify import (
    load_schema,
    _compute_sha256,
    _fail,
    _pass,
...
def test_load_schema_success(tmp_path):
    ...
    with patch("merger.repoground.cli.pr_schau_verify.SCHEMA_PATH", schema_file):
...
def test_load_schema_missing_file(tmp_path, capsys):
    ...
    with patch("merger.repoground.cli.pr_schau_verify.SCHEMA_PATH", missing_file):
        with pytest.raises(SystemExit) as excinfo:
            load_schema()
        assert excinfo.value.code == 1
...
def test_load_schema_invalid_json(tmp_path, capsys):
    ...
    with patch("merger.repoground.cli.pr_schau_verify.SCHEMA_PATH", invalid_file):
        with pytest.raises(SystemExit) as excinfo:
            load_schema()
        assert excinfo.value.code == 1
...
def test_verify_full_zone_dual_read(tmp_path, capsys):
    ...
    from merger.repoground.cli.pr_schau_verify import verify_full
    ...
    def check_content(content):
        ...
        with patch("merger.repoground.cli.pr_schau_verify._compute_sha256", return_value="dummy"):
...
def test_run_verify_invalid_level(capsys):
    ...
    from merger.repoground.cli.pr_schau_verify import run_verify
...
def test_main_verify_dispatches_to_run_verify(tmp_path, monkeypatch):
    ...
    from merger.repoground.cli import pr_schau_verify
