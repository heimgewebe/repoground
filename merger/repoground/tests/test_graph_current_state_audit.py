...
def test_graph_audit_names_existing_contracts_and_surfaces() -> None:
    ...
    for path in (
        "merger/repoground/contracts/architecture.graph.v1.schema.json",
        "merger/repoground/contracts/architecture.graph_index.v1.schema.json",
        "merger/repoground/contracts/entrypoints.v1.schema.json",
        "merger/repoground/architecture/import_graph.py",
        "merger/repoground/architecture/entrypoints.py",
        "merger/repoground/architecture/graph_index.py",
    ):
...
def test_graph_audit_preserves_historical_g3_gap_and_tracks_current_slice() -> None:
    merge_source = _read("merger/repoground/core/merge.py")
    producer = _read("merger/repoground/architecture/bundle_sources.py")
...
def test_graph_audit_keeps_historical_stale_ranking_finding_after_g1_fix() -> None:
    query_source = _read("merger/repoground/retrieval/query_core.py")
...
def test_graph_audit_tracks_exploratory_cli_provenance() -> None:
    cli_source = _read("merger/repoground/cli/cmd_architecture.py")
    audit = AUDIT.read_text(encoding="utf-8")
...
def test_g2_compiler_validates_and_binds_both_sources() -> None:
    compiler = _read("merger/repoground/architecture/graph_index.py")
    validation = _read("merger/repoground/architecture/graph_source_validation.py")
    merge_source = _read("merger/repoground/core/merge.py")
