...
def test_cli_atlas_analyze_growth_error_missing_snapshot(tmp_path, monkeypatch):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "merger.repoground.cli.main", "atlas", "analyze", "growth", "missing1", "missing2"],
...
def test_cli_atlas_analyze_growth_error_incomplete_snapshot(tmp_path, monkeypatch):
    ...
    result = subprocess.run(
        [sys.executable, "-m", "merger.repoground.cli.main", "atlas", "analyze", "growth", "snap_src_1", "snap_tgt_1"],
        capture_output=True, text=True, env=env
    )
    assert result.returncode == 1
