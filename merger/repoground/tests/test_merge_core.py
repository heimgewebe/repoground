...
class TestScanRepoWorktreeExclusion(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Normal repo content
        (self.root / "merger" / "repoground" / "core").mkdir(parents=True)
        (self.root / "merger" / "repoground" / "core" / "merge.py").write_text("# merge")
        # .claude settings — must be included
        (self.root / ".claude").mkdir()
        (self.root / ".claude" / "settings.local.json").write_text("{}")
        # Worktree artefacts — must be excluded
        (self.root / ".claude" / "worktrees" / "foo" / "merger" / "repoground" / "core").mkdir(parents=True)
        (self.root / ".claude" / "worktrees" / "foo" / "merger" / "repoground" / "core" / "merge.py").write_text("# worktree copy")
    ...
    def test_scan_repo_excludes_worktree_files(self):
        ...
        self.assertIn("merger/repoground/core/merge.py", paths)
...
class TestPrescanRepoWorktreeExclusion(unittest.TestCase):
    ...
    def test_prescan_repo_excludes_worktree_subtree(self):
        ...
        self.assertIn("merger/repoground", paths, "Expected 'merger/repoground' in prescan tree")
        self.assertIn("merger/repoground/core", paths, "Expected 'merger/repoground/core' in prescan tree")
        self.assertIn("merger/repoground/core/merge.py", paths, "Expected 'merger/repoground/core/merge.py' in prescan tree")
