...
from merger.repoground.core.merge import FileInfo, scan_repo, write_reports_v2
from merger.repoground.core.output_health import compute_output_health
from merger.repoground.core.post_emit_health import compute_post_emit_health
from merger.repoground.core.pr_delta_cards import SourceValidationError
...
_BUNDLE_MANIFEST_V1_SCHEMA_PATH = _CONTRACTS_DIR / "bundle-manifest.v1.schema.json"
_BUNDLE_MANIFEST_SCHEMA_PATH = _CONTRACTS_DIR / "bundle-manifest.v2.schema.json"
...
def test_generate_bundle_manifest_integration(tmp_path):
    ...
    assert data["kind"] == "repoground.bundle.manifest"
...
def test_dual_bundle_without_sqlite_is_health_fail(tmp_path, monkeypatch):
    monkeypatch.setattr("merger.repoground.core.merge.build_derived_artifacts", lambda *args, **kwargs: [])
...
def test_dual_bundle_jsonschema_missing_still_materializes_sqlite(tmp_path, monkeypatch):
    monkeypatch.setattr("merger.repoground.core.range_resolver.jsonschema", None)
...
def test_manifest_coherence_check_accepts_coherent_manifest(tmp_path):
    from merger.repoground.core.citation_map import check_manifest_coherence_for_citation_map
...
_BUNDLE_SCHEMA = json.loads(
    _BUNDLE_MANIFEST_V1_SCHEMA_PATH.read_text(encoding="utf-8")
)
...
def test_claim_evidence_map_unexpected_missing_with_registry(tmp_path, monkeypatch):
    ...
    import merger.repoground.core.claim_evidence_map as cem
