...
from merger.repoground.retrieval.session import build_agent_query_session_v2
...
def test_build_session_bundle_status_not_dict_logs_warning(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="merger.repoground.retrieval.session"):
...
def test_build_agent_query_session_v1_backward_compatibility():
    ...
    from merger.repoground.retrieval.session import build_agent_query_session
    try:
        _require_module()
    except RuntimeError:
        pytest.skip("jsonschema not available")
