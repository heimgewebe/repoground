...
runs before an RepoGround scan/merge. It is classified in ``docs/service-api.md`` as a
...
# When the *running RepoGround code repo itself* reaches one of these states, the
...
def is_self_repo(repo_path: Path) -> bool:
    """True if ``repo_path`` contains the running RepoGround code or the process CWD.
