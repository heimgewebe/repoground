"""Latest-complete RepoGround bundle registry.
...
def _latest_complete_eligibility(
    ...
    *,
    ...
    record(
        "manifest_kind",
        is_bundle_manifest(manifest),
        manifest.get("kind"),
        (
            f"{CANONICAL_BUNDLE_KIND}@{CANONICAL_BUNDLE_VERSION} or "
            f"{LEGACY_BUNDLE_KIND}@{LEGACY_BUNDLE_VERSION}"
        ),
    )
    commit_valid = (
    ...
    if isinstance(profile, str):
        try:
            ...
            pass
    record("profile_known", profile_known, profile, "known RepoGround profile")
    record(
        "profile_evaluation",
