"""
Context Quality Signals projector for RepoGround bundles (roadmap PR B1).
...
def compute_context_quality(
    ...
    *,
    ...
    manifest cannot be read or is not a supported RepoGround or legacy bundle manifest, returns a
    ...
    if not is_bundle_manifest(manifest):
        return _blocked_report(
            run_id,
            resolved_manifest,
            "manifest is not a supported RepoGround or legacy bundle manifest; cannot project context quality",
            bundle_run_id=bundle_run_id,
        )
