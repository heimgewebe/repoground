...
document that an LLM agent reads FIRST to understand a RepoGround bundle:
...
def render_agent_reading_pack(model: PackModel) -> str:
    ...
    lines.append("- CLI: `python3 -m merger.repoground.cli.main agent-consumption preflight --task-profile <profile> ...`")
    lines.append(
    ...
    lines.append(
        "- Snapshot Plan reports describe how RepoGround selected the snapshot profile "
    ...
    lines.append("```text")
    lines.append("RepoGround consumption:")
    ...
    if model.sqlite_index_path:
        ...
        lines.append("```bash")
        lines.append(
            "python3 -m merger.repoground.cli.main query "
    ...
    lines.append("```bash")
    lines.append(
        "python3 -m merger.repoground.cli.main range get "
