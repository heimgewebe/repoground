"""
Post-emit Bundle Health validator for RepoGround bundles (roadmap PR A4).
...
from .bundle_identity import bundle_schema_path, is_bundle_manifest
...
def _validate_manifest_schema(manifest: Dict[str, Any]) -> Tuple[str, str]:
    """Returns (status, message): pass | fail | environment_error."""
    schema_path = bundle_schema_path(manifest)
    if jsonschema is None:
...
def compute_post_emit_health(
    ...
    *,
    ...
    if not isinstance(manifest, dict) or not is_bundle_manifest(manifest):
        return _assemble(
            status="blocked",
            run_id=run_id,
            bundle_run_id=bundle_run_id,
            manifest_path_str=manifest_path_str,
            checks=[_check("manifest_present", "blocked", "not a supported RepoGround or legacy bundle manifest")],
            errors=["manifest is not a supported RepoGround or legacy bundle manifest; cannot certify bundle surface"],
            warnings=[],
            jsonschema_available=jsonschema is not None,
        )
