...
def _scan_index(
    ...
    *,
            ...
            with mmap.mmap(handle.fileno(), 0, access=mmap.ACCESS_READ) as data:
                ...
                while True:
                    ...
                    if data[position] == 0x7D:
                        ...
                        break
                    key_start = position
                    key_end = _scan_string_end(data, key_start, end)
                    key = _decode_slice(data, key_start, key_end, label="call graph key")
                    ...
                    position = _skip_ws(data, position + 1, end)
                    if key == "calls":
                        if calls_seen:
                        ...
                        while True:
                            ...
                            raw = _decode_slice(
                                data, call_start, call_end, label="call graph call record"
                            )
                            if not isinstance(raw, dict) or not bundle_access._call_record_is_valid(raw):
                                raise ValueError(
                                    f"python_call_graph_json call record at index {len(call_starts)} is invalid"
                                )
                            if len(call_starts) >= MAX_CALLS:
                                raise ValueError(
                                    "python_call_graph_json exceeds bounded impact-index call count"
                                )
                            call_position = len(call_starts)
                            call_starts.append(call_start)
                            call_ends.append(call_end)
                            _append_posting(
                                caller_positions,
                            ...
                            resolution_counts[raw["resolution_status"]] += 1
                            evidence_counts[raw["evidence_level"]] += 1
                            relation_counts[raw["relation_type"]] += 1
    ...
    if _identity(before) != _identity(after) or _identity(after) != fingerprint.artifact_identity:
    ...
    identity_error = bundle_access._call_graph_identity_error(metadata)
    if identity_error is not None:
        raise ValueError(str(identity_error.get("error")))
    model_error = bundle_access._call_graph_model_error(metadata)
    if model_error is not None:
        raise ValueError(str(model_error.get("error")))
    _validate_stream_counts(
        metadata,
        call_count=len(call_starts),
        resolution_counts=resolution_counts,
        evidence_counts=evidence_counts,
        relation_counts=relation_counts,
    )
    binding_error = bundle_access._call_graph_manifest_binding_error(
        metadata, Path(fingerprint.manifest_path), manifest=manifest
    )
    ...
    return _ImpactIndexState(
        ...
        resolved_target_positions=resolved_target_positions,
        candidate_target_positions=candidate_target_positions,
        simple_name_positions=simple_name_positions,
