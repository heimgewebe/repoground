# REPOGROUND BUILD SPEC v2.4

(Normative Spezifikation für RepoGround 3.x; der Report-Contract bleibt v2.4)

> **Kompatibilitätsgrenze:** Persistierte Kennungen wie `repolens-report`,
> `repolens-report.schema.json`, `.repolens-hub-path.txt` und bestehende
> `repolens-*` Skriptnamen bleiben versionierte 2.x/v1-Verträge. Sie sind keine
> aktuellen Produktnamen und dürfen ohne neuen Contract nicht still umbenannt werden.
...
RepoGround build erzeugt aus Working-Copy-Repositories KI-optimierte, strukturierte Repository-Bundles.
...
> `merger/repoground/core/merge.py` umgesetzt (`class ExtrasConfig`,
...
> `repoground-build-spec-super-merger-extras` überwacht.
...
Jeder RepoGround-Build-Report implementiert den formalen Legacy-Contract `repolens-report` v2.4.
Ziel:
...
RepoGround build speichert den Hub-Pfad aus Kompatibilitätsgründen weiterhin in `.repolens-hub-path.txt` im Skript-Verzeichnis. Der Dateiname ist ein persistierter 2.x-Vertrag.

**Hub setzen:**
1. Öffne den gewünschten Hub-Ordner (z. B. `wc-hub`) in Pythonista als Working Directory.
2. Führe den bestehenden 3.x-Kompatibilitätsstarter `repolens-hub-pathfinder.py` aus.
3. Starte den bestehenden 3.x-Kompatibilitätsstarter `repolens.py` neu; die kanonische Oberfläche heißt RepoGround build.
