...
def explain_primary_lens(path: str | Path) -> tuple[str, str]:
    ...
    elif name.startswith("validate_") or "validation" in path_str:
        if lens == "guards":
    ...
    elif name in ("structs.rs", "types.ts", "models.py"):
        if lens == "data_models":
            matched_rule = "data_models: canonical model filename"
    elif "pipelines" in parts or "jobs" in parts or "orchestration" in parts:
        if lens == "pipelines":
    ...
    elif "frontends" in parts or "cli" in parts or "bin" in parts:
        if lens == "entrypoints":
            matched_rule = "entrypoints: frontends/cli/bin path segment"
    elif name in {"__main__.py", "main.rs", "index.ts", "index.js"}:
        if lens == "entrypoints":
            matched_rule = "entrypoints: canonical entrypoint filename"
    elif name.startswith("run_") or name.startswith("start_") or name == "manage.py":
        if lens == "entrypoints":
