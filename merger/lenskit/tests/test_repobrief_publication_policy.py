...
def test_incomplete_ttl_expires_after_48_hours_unless_pinned(tmp_path: Path) -> None:
    ...
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
        now=now,
    )
...
def test_apply_prunes_only_payload_and_preserves_durable_evidence(tmp_path: Path) -> None:
    store = make_store(tmp_path)
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    first_record, first_payload, _ = create_success(
        store, index=41, completed_at=base - dt.timedelta(days=2)
    )
    create_success(store, index=42, completed_at=base - dt.timedelta(days=1))
    create_success(store, index=43, completed_at=base)
...
def test_pin_added_after_plan_prevents_apply(tmp_path: Path) -> None:
    store = make_store(tmp_path)
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    old_record, old_payload, _ = create_success(
        store, index=51, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=52, completed_at=base)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
...
def test_stale_record_or_payload_plan_fails_closed(tmp_path: Path) -> None:
    ...
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    old_record, old_payload, _ = create_success(
        store, index=61, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=62, completed_at=base)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
...
def test_crash_after_quarantine_is_reconciled_idempotently(
    ...
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    old_record, old_payload, _ = create_success(
        store, index=71, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=72, completed_at=base)
    policy = RetentionPolicy(recent=1, daily=0, weekly=0)
    plan = store.plan_retention(
...
def test_tampered_plan_hash_fails_before_payload_mutation(tmp_path: Path) -> None:
    store = make_store(tmp_path)
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    _, old_payload, _ = create_success(
        store, index=92, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=93, completed_at=base)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
...
def test_payload_changed_after_plan_fails_closed(tmp_path: Path) -> None:
    ...
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    _, old_payload, _ = create_success(
        store, index=94, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=95, completed_at=base)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
...
def test_pin_added_after_quarantine_restores_payload(
    ...
    base = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    old_record, old_payload, _ = create_success(
        store, index=96, completed_at=base - dt.timedelta(days=1)
    )
    create_success(store, index=97, completed_at=base)
    policy = RetentionPolicy(recent=1, daily=0, weekly=0)
    plan = store.plan_retention(
        "heimgewebe__demo", "main", policy=policy, now=base
    )
    ...
    monkeypatch.setattr(shutil, "rmtree", crash_before_delete)
    with pytest.raises(RuntimeError, match="simulated crash"):
        store.apply_plan(plan, now=base)
    assert not old_payload.exists()
...
def test_pruned_payload_cannot_be_pinned(tmp_path: Path) -> None:
    store = make_store(tmp_path)
    now = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    old_record, _, _ = create_success(
        store, index=102, completed_at=now - dt.timedelta(days=1)
    )
    create_success(store, index=103, completed_at=now)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
...
def test_duplicate_plan_generation_fails_before_mutation(tmp_path: Path) -> None:
    ...
    now = dt.datetime(2026, 7, 15, 12, tzinfo=UTC)
    _, old_payload, _ = create_success(
        store, index=104, completed_at=now - dt.timedelta(days=1)
    )
    create_success(store, index=105, completed_at=now)
    plan = store.plan_retention(
        "heimgewebe__demo",
        "main",
        policy=RetentionPolicy(recent=1, daily=0, weekly=0),
