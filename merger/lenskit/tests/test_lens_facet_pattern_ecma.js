...
const relationSchemaPath = path.resolve(
  __dirname,
...
const relationSchema = JSON.parse(fs.readFileSync(relationSchemaPath, "utf8"));
...
const relationPattern = relationSchema.definitions.repo_path.pattern;
...
assert.equal(
  relationPattern,
  facetPattern,
  "relation-card path pattern must exactly match lens-facet path pattern",
);
...
const regexes = [
  ...
  ["relation-card", new RegExp(relationPattern, "u")],
];
