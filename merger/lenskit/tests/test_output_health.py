...
# ── A2: excluded_noise diagnostic ────────────────────────────────────────────

def test_excluded_noise_diagnostic_is_present_when_provided(tmp_path):
    """excluded_noise in checks is diagnostic-only: count + paths, no verdict impact."""
    kwargs = _base_kwargs(tmp_path=tmp_path, with_sqlite=False)
    noise_paths = [".pytest_cache/", ".mypy_cache/", ".ruff_cache/"]
    result = compute_output_health(**kwargs, excluded_noise_paths=noise_paths)

    assert "excluded_noise" in result["checks"]
    diag = result["checks"]["excluded_noise"]
    assert diag["status"] == "diagnostic"
    assert diag["count"] == 3
    assert diag["paths"] == noise_paths
    # Must not affect verdict or errors
    assert result["errors"] == []
    assert result["verdict"] in ("pass", "warn")


def test_excluded_noise_diagnostic_absent_when_not_provided(tmp_path):
    """excluded_noise is absent from checks when the parameter is omitted."""
    kwargs = _base_kwargs(tmp_path=tmp_path, with_sqlite=False)
    result = compute_output_health(**kwargs)
    assert "excluded_noise" not in result["checks"]


def test_excluded_noise_diagnostic_empty_list(tmp_path):
    """excluded_noise with empty list records count=0 and is still diagnostic-only."""
    kwargs = _base_kwargs(tmp_path=tmp_path, with_sqlite=False)
    result = compute_output_health(**kwargs, excluded_noise_paths=[])
