...
assert(
    html.includes('Browser hard refresh only — does not restart rLens service'),
    'hard refresh button title explicitly says it does not restart rLens'
);
assert(
    html.includes('id="restartServiceBtn"') && html.includes('Restart rLens'),
    'index.html contains a separate Restart rLens admin button'
);
assert(
...
function createContext() {
    ...
    const elements = {
        ...
        status: makeEl(),
        restartServiceBtn: makeEl({ textContent: 'Restart rLens', className: 'hidden' }),
...
async function run() {
    {
        ...
        assert(
            ctx.confirmMessage === 'Clear browser cache/storage and reload the UI? This does not restart the rLens service.',
